...
function calcRecipe(
  ...
  // ── 1. LUCRO TEÓRICO (Necessário para o objetivo) ──────
  const targetProfit = totalCost * (marginRate / Math.max(1 - marginRate, 0.01));

  // ── 2. OBJETIVO (A META DE VENDA) ───────────────────
  // Calculamos o que seria o objetivo HOJE com os custos atuais
  const objetivoCalculado = totalCost + targetProfit;

  // A TRAVA: Se a receita está guardada e tem um valor antigo, 
  // mantemos esse valor para ele não "dançar" com o armazém.
  let objetivo = (isSaved && storedObjetivo > 0) 
    ? storedObjetivo 
    : (objetivoCalculado > 0.01 ? objetivoCalculado : 0);

  // ── 3. DOSES E FATURAÇÃO ────────────────────────────
  // Agora as doses são calculadas com base no objetivo "congelado"
...
useEffect(() => {
  // 🔴 Se a receita já existe (está guardada), NÃO sincronizamos com o armazém
  // Isso impede que o preço guardado seja esmagado pelo preço "vivo"
  if (!!existing) return; 
...
}, [warehouseSaved, existing]); // Adicionamos 'existing' às dependências
  ...
  requirePro(() => {
    ...
    // USAMOS O VALOR QUE ESTÁ NO ESTADO AGORA
    const data: SavedRecipe = {
      key,
      name,
      date: new Date().toISOString(),
      active: true,
      sellPrice: sellPrice, // Força o preço atual
      margin: margin,     // Força a margem atual
      totalCost: calc.totalCost,
      profit: calc.lucroReal,
      objetivo: calc.objetivo, // <-- O VALOR "CONGELADO"
      ingredients: ingredients.map(ing => ({
        ...ing,
        price: ing.price // <-- O PREÇO DO ARMAZÉM AGORA
      })),
      extras: typeof extras === 'number' ? extras : 0,
      loss,
      fryer: fryerOn,
      fryerData: { ...fryerData },
      energy: energyOn,
